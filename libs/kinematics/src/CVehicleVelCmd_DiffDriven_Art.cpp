...
IMPLEMENTS_SERIALIZABLE(CVehicleVelCmd_DiffDriven_Art, CVehicleVelCmd, mrpt::kinematics)

CVehicleVelCmd_DiffDriven_Art::CVehicleVelCmd_DiffDriven_Art() :
	lin_vel(.0),
    ang_vel(.0)
{
}
...
size_t CVehicleVelCmd_DiffDriven_Art::getVelCmdLength() const
{
    return 2;
}

std::string CVehicleVelCmd_DiffDriven_Art::getVelCmdDescription(const int index) const
{
    switch (index)
    {
...
double CVehicleVelCmd_DiffDriven_Art::getVelCmdElement(const int index) const
{
    switch (index)
    {
    case 0: return lin_vel; break;
...
void CVehicleVelCmd_DiffDriven_Art::setVelCmdElement(const int index, const double val)
{
    switch (index)
    {
...
bool CVehicleVelCmd_DiffDriven_Art::isStopCmd() const
{
    return lin_vel == .0 && ang_vel == .0;
}

void CVehicleVelCmd_DiffDriven_Art::setToStop()
{
    lin_vel = ang_vel = .0;
}

void CVehicleVelCmd_DiffDriven_Art::readFromStream(mrpt::utils::CStream &in, int version)
{
    switch (version)
    {
    case 0:
...
void CVehicleVelCmd_DiffDriven_Art::writeToStream(mrpt::utils::CStream &out, int *version) const
{
    if (version)
    {
        *version = 0;
...
void CVehicleVelCmd_DiffDriven_Art::cmdVel_scale(double vel_scale)
{
    lin_vel *= vel_scale;
    ang_vel *= vel_scale;
}
...
void CVehicleVelCmd_DiffDriven_Art::filter_max_vw(double &v, double &w, const TVelCmdParams &p)
{
